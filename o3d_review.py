...
<html lang='en'>
  <head>
    <title>%(title)s</title>
    <meta charset='utf-8'>
    <script src='../../three.js-r72/build/three.js'></script>
    <script src='../../three.js-r72/examples/js/loaders/DDSLoader.js'></script>
    <script src='../../ObjectLoaderDdsHack.js'></script>
    <script src='../../three.js-r72/examples/js/controls/OrbitControls.js'></script>
    <script src='../../three.js-r72/utils/exporters/blender/tests/scripts/js/review.js'></script>
    <link href='../../three.js-r72/utils/exporters/blender/tests/scripts/css/style.css' rel='stylesheet' />
  </head>
  <body>
    <script>
      init('%(filename)s');
...
def copy_for_review(tmp_json, tag):
    tag_dir = os.path.join('./review-out', tag)
    ...
    src_tex_dir = os.environ.get("TEXTURE_SRC_DIR", "")
    if src_tex_dir:
        dir_tmp = src_tex_dir
    for texture in textures:
        texture = os.path.join(dir_tmp, texture)
        if not os.path.exists(texture):
            print "MISSING TEXTURE FILE:", texture
            continue
